...
## Implementação inicial (protótipo)
Este repositório inclui um protótipo funcional com:
- **coleta de métricas** (tempo + memória),
- **seleção evolutiva** via função de fitness,
- **reescrita** baseada em AST/geração de código,
- **validação** em sandbox com timeout,
- **histórico** em `.evolution/history.json`.
...
O script testa duas variantes de `process_data` e escolhe a melhor com base nas métricas coletadas.
